...
const ModalConfigLevelConditionsUpdate = ({ onSubmit, onAfterUpdate }, ref) => {
  ...
  const onOpen = (record) => {
    ...
    const conditions = [];
    record.childrenLevelConditions.map((e) => {
      conditions.push(e.type);
      if (e.type === 1) {
        setValue("valueSales", e.value)
        setValue("amountMonthSales", e.amountMonth)
        setValue("noteSales", e.note)
      }
      if (e.type === 2) {
        setValue("valueIntroduce", e.value)
        setValue("unitIntroduce", e.unit)
        setValue("noteIntroduce", e.note)
      }
      if (e.type === 3) {
        setValue("valueLevel", e.value)
        setValue("noteLevel", e.note)
      }
    });

    setConditions(conditions);
    setValue("idLevel", record.idLevel);
  ...
  const submitLevelConditionsUpdate = async (values) => {
    let isReturn = false;
    const body = [];
    ...
    try {
      await Promise.all(
        body.map(async (e) => {
          await levelConditionsApis.updateLevelConditions(e);
        })
      );
    } catch (err) {
      errorHelper(err);
  ...
  return (
    ...
    >
      ...
      <form onSubmit={handleSubmit(submitLevelConditionsUpdate)}>
...
export default forwardRef(ModalConfigLevelConditionsUpdate);
