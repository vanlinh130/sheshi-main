...
const ModalConfigCommissionCreate = ({ onSubmit, onAfterUpdate }, ref) => {
  ...
  const onOpen = (record) => {
    setValue("commissionName", record.commissionName)
    setValue("percent", record.percent)
    setValue("conditions", record.conditions)
    setValue("note", record.note)
    setId(record.id)
    setValueCommission(record.type);
    setVisible(true);
  };
  ...
  const submitConfigDataCreate = async (values) => {
    ...
    return commissionApis
      .updateConfigCommission({ ...values, type: valueCommission, id })
      .then(() => {
